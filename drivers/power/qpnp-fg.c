...
static void tinno_fg_status_trace(struct fg_chip *chip)
{
	...
	if (curr_soc != last_soc)
	{
		...
		printk("FG: BAT T[CHG SOC OCV VC I T]=%d[%d,%d,%d,%d,%d,%d],%lu,%lu\n",
			count,
	...
	{
		printk("FG: BAT[CHG SOC OCV VC I T]=%d[%d,%d,%d,%d,%d,%d]\n",
			count,
			(is_usb_present(chip) ? 1 : 0), 		// plug/unplug
			curr_soc,								// %
			fg_data[FG_DATA_OCV].value/1000,		// mV
...
out:
	...
	// pony.ma, DATE20170309, optimize log mesages, DATE20170309-01 START
	pr_info("BATT_TEMP = %d,OCV = %d,VOLTAGE = %d,CURRENT = %d,BATT_SOC = %d\n", 
		fg_data[0].value,fg_data[1].value,fg_data[2].value,fg_data[3].value,fg_data[6].value);
	msoc = get_monotonic_soc_raw(chip);
	nsoc = DIV_ROUND_CLOSEST((msoc - 1) * (FULL_CAPACITY - 1),FULL_SOC_RAW - 2) + 1;
	pr_info("real soc = %d,msoc=%d\n",nsoc,msoc);
	// pony.ma, DATE20170309-01 END
