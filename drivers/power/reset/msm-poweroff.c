...
//++ write misc for ramdump TN:peter
//#define TINNO_MISC_RAMDUMP_SUPPORT
//-- write misc for ramdump 
...
//++ write misc for ramdump TN:peter
#ifdef TINNO_MISC_RAMDUMP_SUPPORT
#define MISC_PATH  "/dev/block/bootdevice/by-name/misc"
#define OFFSET_FLAG  4096
#include <linux/fs.h>
#include <linux/uaccess.h>

static void write_misc_for_ramdump(int flag)
{
        mm_segment_t fs;   
        char misc_data[10] = {0};
        char *enable = "enable";
		char *disnable = "disable";
        struct file *ps_filp = NULL;

		if(flag == 1)
			strcpy(misc_data,enable);
		else			
			strcpy(misc_data,disnable);
        ...
        else
        {
            ...
            ps_filp->f_op->write(ps_filp, (char*)misc_data, strlen(misc_data), &ps_filp->f_pos);
...
static int dload_set(const char *val, struct kernel_param *kp)
{
	...
	#ifdef TINNO_MISC_RAMDUMP_SUPPORT
    if(download_mode == 1)
		write_misc_for_ramdump(1);
	else
		write_misc_for_ramdump(0);
	#endif
	//-- write misc for ramdump
...
//ramdump control TN:peter	
#ifdef CONFIG_TINNO_KE_LOG_CTRL
	char *temp = module_parser_mask("msmpoff");
	if(temp != NULL)
	{
		printk("%s, temp = %s\n",__func__,temp);
		if(!strncmp(temp,"1",1))
