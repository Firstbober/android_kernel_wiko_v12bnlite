...
void __ref tinno_hotplug_trace(char *flag)
{
    char buf[32];
    int n = cpulist_scnprintf(buf, 32-2, cpu_online_mask);
    int num = cpumask_weight(cpu_online_mask);

    buf[n++] = '\n';
    buf[n] = '\0';

    printk("T_PERF %s online:%d, CPU%s", flag, num, buf);
}
EXPORT_SYMBOL(tinno_hotplug_trace);
...
static void __ref do_core_control(long temp)
{
	...
	mutex_unlock(&core_control_mutex);
	#if defined (CONFIG_TINNO_CPU_TRACE)
	//tinno_hotplug_trace("check_temp");
	#endif  /* CONFIG_TINNO_CPU_TRACE */
}
...
static int __ref update_offline_cores(int val)
{
	...
	#if defined (CONFIG_TINNO_CPU_TRACE)
	tinno_hotplug_trace("update_offline_cores");
	#endif  /* CONFIG_TINNO_CPU_TRACE */
