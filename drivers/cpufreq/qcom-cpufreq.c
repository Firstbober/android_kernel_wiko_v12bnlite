...
// Jake.L, DATE20170210, NOTE, DATE20170210-01 START
#if defined (CONFIG_TINNO_CPU_TRACE)
#define CPUFREQ_PHARSE_TIME     5000//1000    // ms
struct cpufreq_pharse{
    unsigned int cur_freq;  // MHz
    unsigned int max_freq;	// Mhz
    unsigned int power;     // freq * time
    s64 last_time;          // last update cpu pharse information time, ms
    s64 last_output_time;   // last output cpu pharse information time, ms
};

static struct cpufreq_pharse cpu_pharse_info[CONFIG_NR_CPUS];
static int cpufreq_pharse_output_flag = 1;
static void tinno_cpufreq_init_max_freq(unsigned int cpu, unsigned int freq)
{
	unsigned int freq_mhz = freq / 1000;
	
	//printk("T_PERF cpu_f CPU%d max freq is %dMHz\n", cpu, freq_mhz);
	if (cpu < 4)
	{
	    cpu_pharse_info[0].max_freq = freq_mhz;
	    cpu_pharse_info[1].max_freq = freq_mhz;
	    cpu_pharse_info[2].max_freq = freq_mhz;
	    cpu_pharse_info[3].max_freq = freq_mhz;
	}
	#if (CONFIG_NR_CPUS > 4)
	else if(cpu < 8)
	{
	    cpu_pharse_info[4].max_freq = freq_mhz;
	    cpu_pharse_info[5].max_freq = freq_mhz;
	    cpu_pharse_info[6].max_freq = freq_mhz;
	    cpu_pharse_info[7].max_freq = freq_mhz;
	}
	#endif  /* CONFIG_NR_CPUS */
}

void tinno_cpufreq_trace(struct cpufreq_policy *policy,
				  unsigned int target_freq)
{
    s64 now, delta_time;
    unsigned int i, cpu = policy->cpu;
    static int init_flag = 0;

    /* function check. */
    if (!cpufreq_pharse_output_flag)
        return ;
    ...
    if (!init_flag)
    {
        ...
        {
            cpu_pharse_info[i].cur_freq = 0;
			if (!cpu_pharse_info[i].max_freq)
				cpu_pharse_info[i].max_freq = 1000;	// default value is 1000MHz.
            cpu_pharse_info[i].power = 0;
            cpu_pharse_info[i].last_time = now;
            cpu_pharse_info[i].last_output_time = now;
        }
        init_flag = 1;
    }

    // update the cpu pharse information.
    delta_time = now - cpu_pharse_info[0].last_time;
    //printk("T_PERF cpu_f CPU%d: %dKHz, delta_time=%d\n", cpu, target_freq, (unsigned int)delta_time);
    if (delta_time)
	{
	    for (i = 0; i < CONFIG_NR_CPUS; i++)
	    {
	        ...
	        {
	            // calculate power.
	            cpu_pharse_info[i].power += cpu_pharse_info[i].cur_freq * delta_time;
	        }
	    }
	    cpu_pharse_info[0].last_time = now;
	}
	
	#if 0
	cpu_pharse_info[cpu].cur_freq = target_freq / 1000;
	#else
	if (cpu < 4)
	{
		unsigned int freq_mhz = target_freq / 1000;
	    cpu_pharse_info[0].cur_freq = freq_mhz;
	    cpu_pharse_info[1].cur_freq = freq_mhz;
	    cpu_pharse_info[2].cur_freq = freq_mhz;
	    cpu_pharse_info[3].cur_freq = freq_mhz;
	}
	#if (CONFIG_NR_CPUS > 4)
	else if(cpu < 8)
	{
		unsigned int freq_mhz = target_freq / 1000;
	    cpu_pharse_info[4].cur_freq = freq_mhz;
	    cpu_pharse_info[5].cur_freq = freq_mhz;
	    cpu_pharse_info[6].cur_freq = freq_mhz;
	    cpu_pharse_info[7].cur_freq = freq_mhz;
	}
	#endif  /* CONFIG_NR_CPUS */
	#endif  /* #if 0 */

    // check whether output the cupu pharse information.
    delta_time = now - cpu_pharse_info[0].last_output_time;
    if (delta_time >= CPUFREQ_PHARSE_TIME)   // output the cpu pharse information.
    {
        //printk("T_PERF cpu_f delta_time=%d\n", (unsigned int)delta_time);
        printk("T_PERF qcom_cpuf CPU0-%d", CONFIG_NR_CPUS-1);
        for (i = 0; i < CONFIG_NR_CPUS; i++)
        {
            #if 0
            printk("CPU%d:%d ", 
                i, (unsigned int)(cpu_pharse_info[i].power/delta_time));
			#else
            printk(" %d", 
                (unsigned int)(cpu_pharse_info[i].power/delta_time)*100/cpu_pharse_info[i].max_freq);
            #endif  /* #if 0 */

            // reset for next pharse
            cpu_pharse_info[i].power = 0;
            cpu_pharse_info[i].last_time = now;
        }
        #if 0
        ...
        printk("\n");
        #endif  /* #if 0 */
        cpu_pharse_info[0].last_output_time = now;
    }
}
EXPORT_SYMBOL(tinno_cpufreq_trace);
#endif  /* CONFIG_TINNO_CPU_TRACE */
// Jake.L, DATE20170210-01 END
...
static int msm_cpufreq_init(struct cpufreq_policy *policy)
{
	...
	#if defined (CONFIG_TINNO_CPU_TRACE)
	//printk("T_PERF qcom_cpuf init CPU[%d] %dkHz, [%d - %dkHz]\n", 
	//	policy->cpu, table[index].frequency, policy->min, policy->max);
	tinno_cpufreq_init_max_freq(policy->cpu, policy->max);
	#endif  /* CONFIG_TINNO_CPU_TRACE */
