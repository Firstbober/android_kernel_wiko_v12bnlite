...
// pony.ma, DATE20170120, realize nonstandard charger, DATE20170120-01 LINE
...
static void msm_chg_check_dcd_flchg(struct msm_otg *motg)
{
	...
	msm_chg_disable_dcd(motg);
	msm_chg_block_off(motg);
	printk ("msm_chg_check_dcd_flchg: DCD:%d chg_type:%d\n",
...
static void msm_otg_sm_work(struct work_struct *w)
{
	...
	pr_err("otg phy state = %d\n", otg->phy->state); 
	pr_err("otg chg state = %d\n", motg->chg_state); 
	pr_err("otg chg type = %d\n", motg->chg_type);
		...
		if (!test_bit(ID, &motg->inputs) && otg->host) {
			...
			case USB_CHG_STATE_DETECTED:
				...
				case USB_UNSUPPORTED_CHARGER:
					printk("UNSUPPORTED charger present \n");
