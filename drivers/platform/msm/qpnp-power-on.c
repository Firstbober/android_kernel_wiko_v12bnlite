...
#define QPNP_PON_GEN2_DELAY_BIT_SHIFT		14   //qcom patch debounce
...
struct qpnp_pon {
	...
	bool			store_hard_reset_reason;
      bool kpd_dbc_enable;       //qcom patch debounce
      ktime_t kpd_release_time;     //qcom patch debounce
};
...
static int qpnp_pon_get_dbc(struct qpnp_pon *pon, u32 *delay)   //qcom patch debounce
{ 
     int rc; 
     u8 val; 


     rc = spmi_ext_register_readl(pon->spmi->ctrl, pon->spmi->sid, QPNP_PON_DBC_CTL(pon),  &val, 1);
     if (rc) { 
     ...
     if (is_pon_gen2(pon)) 
        *delay = USEC_PER_SEC / (1 << (QPNP_PON_GEN2_DELAY_BIT_SHIFT - val)); 
     else 
...
qpnp_pon_input_dispatch(struct qpnp_pon *pon, u32 pon_type)
{
	...
	u32 key_status;
      u64 elapsed_us;    //qcom patch debounce
	...
	if (!cfg->key_code)
        ...
        if (pon->kpd_dbc_enable && cfg->pon_type == PON_KPDPWR) {   //qcom patch debounce
            elapsed_us = ktime_us_delta(ktime_get(), pon->kpd_release_time); 
            if (elapsed_us < pon->dbc) { 
                pr_debug("Ignoring kpd event - within debounce time\n"); 
                return 0; 
            } 
        } 
...
static int qpnp_pon_probe(struct spmi_device *spmi)
{
	...
	if (rc) {
      ...
      qpnp_pon_get_dbc(pon, &pon->dbc);     //qcom patch debounce
      
      pon->kpd_dbc_enable = of_property_read_bool(pon->spmi->dev.of_node,  //qcom patch debounce
      "qcom,kpd-dbc-enable"); 
      printk("qpnp_pon_probe: kpd-dbc-enable %d\n", pon->kpd_dbc_enable);
