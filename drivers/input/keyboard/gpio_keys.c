...
static struct wake_lock camera_gpiokey_wake_lock;          //tinno sugar key
/*
...
static void gpio_keys_gpio_report_event(struct gpio_button_data *bdata)
{
	...
	if((button->code == 252)  && (state ==1)  )       //tinno sugar key          252
	{
		wake_lock_timeout(&camera_gpiokey_wake_lock, msecs_to_jiffies(3000));
		printk("tinno sugar key press, wake lock system for 3000ms\n");
	}
}
...
static int gpio_keys_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < pdata->nbuttons; i++) {
	 ...
	 wake_lock_init(&camera_gpiokey_wake_lock, WAKE_LOCK_SUSPEND, "camera_gpio_key_wake_lock");   //tinno sugar key
