...
void fts_irq_disable_tinno(struct fts_ts_data *data)
{
...
void fts_irq_enable_tinno(struct fts_ts_data *data)
{
...
static void fts_touch_irq_work(struct work_struct *work)
{
    ...
    fts_irq_enable_tinno(fts_wq_data);   //tinno
...
//    if(fts_ts ->suspended == false) {  //FTS_GESTURE_EN  not support
		fts_irq_disable_tinno(fts_ts);          //tinno
...
//END<REQ><><20150910>Add WAKEUP_GESTURE for ft5xx;xiongdajun
	...
	if (ret < 0) {
		WARN(true, "read touchdata failed.\n");
...
*  Return: 
*******************************************************************************/
int  TINNO_TP_BORDER_TOUCH = 12;    // tinnotp border touch function
...
err_irq_gpio_dir:
	if (gpio_is_valid(data->pdata->irq_gpio))
		gpio_free(data->pdata->irq_gpio);
err_irq_gpio_req:
	WARN(true, "fts_gpio_configure failed.\n");
	return err;
}
...
static int fts_ts_start(struct device *dev)
{
	...
	msleep(data->pdata->soft_rst_dly);
     ...
     //   enable_irq(data->client->irq);
       fts_irq_enable_tinno(data); //tinno
...
//	disable_irq(data->client->irq);
    //    disable_irq_nosync(data->client->irq);
    fts_irq_disable_tinno(data);   //tinno
...
//Begin<REQ><><20150910>Add WAKEUP_GESTURE for ft5xx;xiongdajun
	...
	if (bEnTGesture) {
		...
		dev_err(dev, "Enter into gesture mode.\n");
		return 0;
	}
#endif
#endif
...
static int sensitive_notifier_callback(struct notifier_block *nb,
		unsigned long act, void *data)
{
	pr_info("fts high sensitive enable = %ld\n", act);
	fts_write_reg(fts_i2c_client, 0xc1, act);
	return 0;
}

static int fts_sensitive_init(struct fts_ts_data* fts)
{
	fts->sensitive_nb.notifier_call = sensitive_notifier_callback;
	sensitive_notifier_register(&fts->sensitive_nb);
	return 0;
}
...
static int fts_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	fts_input_dev = input_dev;
       ...
       spin_lock_init(&data->irq_lock);   //tinno
       INIT_WORK(&data->touch_event_work, fts_touch_irq_work);  //tinno
       data->ts_workqueue = create_workqueue(FTS_WORKQUEUE_NAME);
        if (!data->ts_workqueue)
        {
            err = -ESRCH;
            dev_err(&client->dev, "Create touch workqueue failed");
...
static int fts_ts_remove(struct i2c_client *client)
{
	...
	debugfs_remove_recursive(data->dir);
        ...
        cancel_work_sync(&data->touch_event_work);    //tinno
	...
	cancel_work_sync(&data->fb_notify_work);
#elif defined(CONFIG_HAS_EARLYSUSPEND)
