...
static int  TINNO_TP_BORDER_TOUCH =24;    // tinnotp border touch function
static ssize_t tinno_tp_border_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    int count;

    printk("tinno_tp_border_store  TINNO_TP_BORDER_TOUCH =%d ", TINNO_TP_BORDER_TOUCH);
    count = sprintf(buf, "border %d\n" ,TINNO_TP_BORDER_TOUCH);
    return count;
}

static ssize_t tinno_tp_border_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
   int tp_border;
   tp_border =simple_strtol(buf, NULL, 10);

//   mutex_lock(&(rmi4_data->rmi4_report_mutex));
   if((tp_border<200)&&(tp_border>=0)){
   	TINNO_TP_BORDER_TOUCH = tp_border;
	printk("tinno_tp_border_store  TINNO_TP_BORDER_TOUCH =%d ", TINNO_TP_BORDER_TOUCH);
   }
...
//static DEVICE_ATTR(tinno_tp_border, S_IRUGO|S_IWUSR, tinno_tp_border_show, tinno_tp_border_store);
/*************tinnotp border touch function****************/
...
static struct device_attribute attrs[] = {
	...
	__ATTR(tinno_tp_border, (S_IRUGO | S_IWUSR | S_IWGRP),     //tinnotp border touch function
			tinno_tp_border_show,
			tinno_tp_border_store),
...
//		printk("tinnotp border touch function  x=%d, y=%d ,sensor_max_x=%d\n", x ,y,rmi4_data->sensor_max_x);
		if(TINNO_TP_BORDER_TOUCH > 0){
			if(x > (rmi4_data->sensor_max_x -TINNO_TP_BORDER_TOUCH))
			{
				if(x > (rmi4_data->sensor_max_x -TINNO_TP_BORDER_TOUCH/2))
				{
				...
				{
					x= x + x +TINNO_TP_BORDER_TOUCH -rmi4_data->sensor_max_x;
					printk("tinnotp border touch function  x=%d\n", x);
				}
			}

			if((x< TINNO_TP_BORDER_TOUCH)&&( y > 0))
			{
				if(x < (TINNO_TP_BORDER_TOUCH/2))
				{
				...
				{
					x = x + x -TINNO_TP_BORDER_TOUCH;
					printk("tinnotp border touch function  x=%d\n", x);
				}
			}      //tinno add for 9031 TP    IIADBHQYH-334
		}
		/*************tinnotp border touch function****************/
...
#ifdef FORCE_INTO_F35
static int synaptics_rmi4_set_into_f35(struct synaptics_rmi4_data *rmi4_data)
{
	int retval;
...
static int synaptics_rmi4_probe(struct platform_device *pdev)
{
...
#ifdef CONFIG_PROJECT_P7771
	{
		#define PINCTRL_STATE_ACTIVE	"pmx_ts_active"
		struct pinctrl *ts_pinctrl;
		struct pinctrl_state *pinctrl_state_active;
		ts_pinctrl = devm_pinctrl_get(pdev->dev.parent);
...
#ifdef FORCE_INTO_F35
	retval = synaptics_rmi4_set_into_f35(rmi4_data);
	if (retval < 0) {
		dev_err(rmi4_data->pdev->dev.parent,
				"%s: Failed to set into f35\n",
				__func__);
		return retval;
	}
	...
	proc_symlink("tinno_tp_border", NULL, "/sys/class/input/input1/tinno_tp_border");       //tinnotp border touch function
