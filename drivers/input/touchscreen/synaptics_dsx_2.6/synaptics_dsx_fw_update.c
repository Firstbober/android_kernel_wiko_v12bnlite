...
#define FW_IHEX_NAME "PR2531076-td4310.ihex.img"
//#define FW_IMAGE_NAME "synaptics/startup_fw_update.img"
#define FW_IMAGE_NAME "PR2531076-td4310.img"
...
static int save_tp_info(char *product_id, char *config_id, int id, int rev_id)
{
	...
	if (!strncmp(product_id, "TTOUCH", sizeof("TTOUCH"))) {
		...
		sprintf(buf, "BOE-%s-TD4310-V%d%d",
			CONFIG_PRODUCT_NAME, config_id[2]-0x30, config_id[3]-0x30);
#else
		sprintf(buf, "BOE-%s-S%d-V%d%d",
			CONFIG_PRODUCT_NAME, id, config_id[2]-0x30, config_id[3]-0x30);
#endif
	}
...
exit:
	...
	save_tp_info(rmi4_data->rmi4_mod_info.product_id_string,
		fwu->config_id, 0/*fwu->rmi4_data->rmi4_mod_info.package_id*/, 0);
...
static int synaptics_rmi4_fwu_init(struct synaptics_rmi4_data *rmi4_data)
{
	...
	sprintf(upgrade_path, "/sys/class/input/%s/doreflash",
